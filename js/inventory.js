$(document).ready(function() {
    ...
    $("#car-search").keyup(function() {
        ...
        if ($('#main-section > div:visible').length === 1) {
            $('.search-error').show();
        } else {
            $('.search-error').hide();
        }
    });
    $('.car-price').click(function() {
        var price = parseInt($(this).text());
        var taxes = (price * .0008).toFixed(2);
        var total = price + parseFloat(taxes);
        alert("price: " + price + ", taxes: " + taxes + ", total: " + total);
    })
});